...
namespace PTXClassLibrary
{
    ...
    public class OE_Return
    {
        ...
        public double[][] arrCommOpt1 = new double[7][];
        ...
        public OE_Return()
        {
            ...
            for (int iCol = 0; iCol < 7; iCol++)
            {
                ...
                {
                    //arrCommOpt1.SetValue(0, iCol, iRow);
                    //arrCommOpt2.SetValue(0, iCol, iRow);
        ...
        //Overload to return an Error
        internal OE_Return(string strErrorType)
        {
    ...
    public class OE_ReturnCollection
    {
        ...
        internal OE_Return[] OEReturnDataArray = new OE_Return[MyCSPGlobals.iOE_MaxNPA - MyCSPGlobals.iOE_MinNPA + 2];

        public OE_ReturnCollection()
        {
            for (int i = 0; i <= MyCSPGlobals.iOE_MaxNPA - MyCSPGlobals.iOE_MinNPA + 1; i++)
            {
                //Starts at age 55 = MyCSPGlobals.iOE_MaxNPA 
                OEReturnDataArray[i] = new OE_Return();
            }
        }


        internal OE_ReturnCollection(string strErrorType)
        {
            for (int i = 0; i <= MyCSPGlobals.iOE_MaxNPA - MyCSPGlobals.iOE_MinNPA + 1; i++)
            {
                //Starts at age 55 = MyCSPGlobals.iOE_MaxNPA 
                OEReturnDataArray[i] = new OE_Return(strErrorType);
            }
        }

        //Return OE_Return Object, uses integers 55-75 or string "SPA"
        public OE_Return OE_Data(int iOE_NPA)
        {
            OE_Return OERet = new OE_Return("Not Found");
            try
            {
                //Convert age to index
                if (iOE_NPA < MyCSPGlobals.iOE_MinNPA || iOE_NPA > MyCSPGlobals.iOE_MaxNPA) { OERet = new OE_Return("NPA out of range"); }
                OERet = OEReturnDataArray[iOE_NPA - MyCSPGlobals.iOE_MinNPA];
            }
            catch
            { }
            return OERet;
        ...
        //Overloaded
        public OE_Return OE_Data(string strOE_NPA)
        {
            OE_Return OERet = new OE_Return("Not Found");
            try
            {
                //if SPA get index 21
                if (strOE_NPA == "SPA") { OERet = OEReturnDataArray[MyCSPGlobals.iOE_MaxNPA - MyCSPGlobals.iOE_MinNPA + 1]; }
                else { OERet = new OE_Return("NPA out of range"); }
            }
            catch
            { }
            return OERet;
